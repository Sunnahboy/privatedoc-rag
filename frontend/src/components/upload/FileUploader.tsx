    ...
    <div className="w-full">
      ...
      {status === "idle" && (
        <div 
          ...
          className={`cursor-pointer rounded-xl border-2 border-dashed border-outline-variant text-center transition-colors hover:bg-surface ${
            compact ? "p-6" : "p-12"
          }`}
        >
          <input
          ...
          <div className="text-on-surface-variant">
            <svg className={`mx-auto mb-4 text-on-surface-variant ${compact ? "h-8 w-8" : "h-12 w-12"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {(status === "uploading" || status === "polling") && (
        <div className="p-8 bg-primary/10 text-primary rounded-xl flex flex-col items-center border border-primary/30">
      ...
      {status === "success" && document && (
        <div className="p-6 bg-success-container text-success rounded-xl border border-success/30">
          ...
          <button onClick={reset} className="mt-4 px-4 py-2 bg-surface-elevated text-success rounded shadow-sm border border-success/30 hover:bg-success-container font-medium transition-colors">
            Upload Another
          </button>
        </div>
      )}
      ...
      {/* 4. ERROR STATE */}
      {status === "error" && (
        <div className="p-6 bg-error/10 text-error rounded-xl border border-error/30">
          <h3 className="font-bold text-lg mb-1">Upload Failed</h3>
          <p>{error}</p>
          <button onClick={reset} className="mt-4 px-4 py-2 bg-surface-elevated text-error rounded shadow-sm border border-error/30 hover:bg-error/10 font-medium transition-colors">
            Try Again
          </button>
        </div>
      )}
