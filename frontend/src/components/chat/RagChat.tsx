...
export function RagChat({
  ...
  const { query, setQuery, isLoading, askQuestion, error, clearChat, chatHistory, handleEditMessage, statusMessage } = useRAGQuery();
  ...
  return (
    <section className={`flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-outline-variant/20 bg-[#FCFBF8] shadow-[0_12px_40px_rgba(15,23,42,0.08)] ${className ?? ""}`}>
          ...
          <>
            ...
            <div className="rounded-2xl border border-outline-variant/20 bg-white/70 px-3 py-2 shadow-sm">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button type="button" onClick={() => runContextAction("Help me understand the main ideas of this book.")} className="rounded-full border border-outline-variant/30 bg-white px-3 py-1.5 text-xs text-on-surface-variant shadow-sm transition-colors hover:bg-surface focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40">Ask about this book</button>
                <button type="button" onClick={() => runContextAction(`Explain the key ideas on page ${currentPage ?? 1}.`)} className="rounded-full border border-outline-variant/30 bg-white px-3 py-1.5 text-xs text-on-surface-variant shadow-sm transition-colors hover:bg-surface focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40">Explain this page</button>
                ...
                <button type="button" onClick={() => runContextAction("Summarize this section in a concise way.")} className="rounded-full border border-outline-variant/30 bg-white px-3 py-1.5 text-xs text-on-surface-variant shadow-sm transition-colors hover:bg-surface focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40">Summarize this section</button>
                <button type="button" onClick={() => runContextAction("Compare this section with another section and explain the key differences.")} className="rounded-full border border-outline-variant/30 bg-white px-3 py-1.5 text-xs text-on-surface-variant shadow-sm transition-colors hover:bg-surface focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40">Compare with another section</button>
              </div>
            </div>
          </>
        )}
      ...
      <div ref={conversationRef} onScroll={handleScroll} className="flex-1 overflow-y-auto bg-[linear-gradient(180deg,#faf8f2_0%,#f7f5ef_100%)] p-4">
          ...
          {chatHistory.map((message) => {
            ...
            return (
              <div key={message.id} className={message.role === "user" ? "group flex justify-end" : "group flex justify-start"}>
                <div className={editingMessageId === message.id ? "relative w-full max-w-[95%]" : message.role === "user" ? "relative w-fit max-w-[88%] rounded-2xl rounded-br-md border border-outline-variant/20 bg-[#faf9f6] px-4 py-3 text-on-surface shadow-sm" : "relative w-fit max-w-[92%] rounded-2xl rounded-bl-md border border-outline-variant/20 bg-[#faf9f6] px-4 py-3 text-on-surface shadow-sm"}>
                      ...
                      {isAiMessage ? (
                        <div className={`relative ${canCollapse && !isExpandedMessage ? "max-h-72 overflow-hidden" : ""}`}>
                          ...
                          {canCollapse && !isExpandedMessage && (
                            <div aria-hidden="true" className="pointer-events-none absolute inset-x-0 bottom-0 h-12 bg-linear-to-t from-white to-transparent" />
                          )}
                        </div>
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="space-y-2">
          ...
          <div className="flex items-end gap-3 rounded-2xl border border-outline-variant/20 bg-surface-container-low/50 p-2 shadow-sm">
            <div className="relative flex-1">
              ...
              />
              <button type="button" disabled className="absolute bottom-2 right-2 inline-flex h-8 w-8 items-center justify-center rounded-full border border-outline-variant/30 bg-white/90 text-on-surface-variant opacity-60 shadow-sm"><span className="material-symbols-outlined text-[18px]">mic</span></button>
            </div>
            <button
              type="button"
              onClick={() => {
              ...
              className={`inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border text-on-primary shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 ${
                isLoading ? "border-red-600 bg-red-600 hover:bg-red-700" : query.trim() ? "border-primary bg-primary hover:bg-primary/90" : "cursor-not-allowed border-outline bg-outline text-on-surface-variant"
              }`}
            >
