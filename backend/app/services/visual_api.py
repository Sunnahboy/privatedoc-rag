...
def get_visual_engine_and_lock(request: Request) -> tuple:
    ...
    if not engine or not lock:
        logger.critical("Visual Engine or Lock was not initialized in application state.")
        raise HTTPException(
...
async def embed_image_endpoint(
    ...
    try:
        ...
        async with lock:
            multi_vector: np.ndarray = await asyncio.to_thread(engine.embed_image, image)
...
async def embed_text_endpoint(
    ...
    try:
        # Concurrency safety: Only 1 request hits the GPU at a time
        async with lock:
            multi_vector: np.ndarray = await asyncio.to_thread(engine.embed_query, payload.text)
        return {"vector": multi_vector.tolist()}
    except Exception as e:
